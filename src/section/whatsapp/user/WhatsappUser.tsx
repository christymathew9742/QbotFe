...
console.log(appointmentData,'list')
  useEffect(() => {
    if (isFetching) {
              ...
              <div className="max-w-full overflow-x-auto">
                <Table>
                  <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                    <TableRow>
                      {["User Name", "Interaction Speed", "Sentiment Score", "Appointments Count", "User Cretated", "User Type", "More"].map((head, i) => (
                        <TableCell key={i} colSpan={2} isHeader className="px-5 py-3  text-theme-xs text-gray-500 dark:text-gray-400 font-medium text-center">
                          {head}
                        </TableCell>
                      ))}
                    </TableRow>
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05] w-full">
                    {isFetching ? (
                      ...
                      appointmentData?.data?.map((user: User) => (
                        <TableRow key={user._id}>
                          <TableCell colSpan={2} className="px-5 py-2 font-light dark:text-white/90 text-theme-sm text-center">
                            {user?.profileName}
                          </TableCell>
                          <TableCell colSpan={2} className="px-5 py-2  font-light dark:text-white/90 text-theme-sm text-center">
                            <span className={`text-xxxs px-2 py-1  rounded-full font-medium text-center ${sentimentScore(user?.sentimentScores?.speedScore)}`}>
                          ...
                          <TableCell colSpan={2} className="px-5 py-2 text-center font-light dark:text-white/90 text-theme-sm">
                            <span className={`text-xxxs px-2 py-1 rounded-full font-medium text-center ${sentimentScore(user?.sentimentScores?.sentimentScore)}`}>
                          ...
                          <TableCell colSpan={2} className="px-3 py-2 text-gray-500 text-theme-sm dark:text-gray-400">
                            <div className="flex justify-center items-center">
                              <span className={`text-xxxs px-1 py-0 rounded-full font-medium block w-[50px] text-center ${userType(user?.userType)}`}>
                                {user?.userType}
                              </span>
                            </div>
                          </TableCell>
