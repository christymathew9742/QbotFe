...
//   // --- 1. Define Status Check (Reusable) ---
...
//     checkGoogleStatus(); // Check status on mount
//   },[dispatch, pendingStatus, checkGoogleStatus]);

//   // --- 3. Handle Google Redirect Params ---
//   useEffect(() => {
//     const googleStatusParam = searchParams.get("status");
//     const errorMessage = searchParams.get("message");

//     if (googleStatusParam) {
//       if (googleStatusParam === "connected") {
//         toast.success("Google Calendar connected successfully!");
//         checkGoogleStatus(); // Refresh UI immediately
//       } else if (googleStatusParam === "disconnected" || googleStatusParam === "error") {
//         const msg = errorMessage ? decodeURIComponent(errorMessage) : "Failed to connect Google Calendar.";
//         toast.error(msg);
//       }

//       // Remove query params to prevent toast loop on refresh
//       router.replace(`${pathname}?tab=${tabValue}`, { scroll: false });
//     }
//   }, [searchParams, router, pathname, tabValue, checkGoogleStatus]);
...
//       timezone: userData.timezone || systemTimezone, 
...
// --- Helper Component to Watch for Timezone Changes ---
// This allows us to trigger warnings if the user interacts with the timezone field
const TimezoneChangeWatcher = () => {
  const { values, initialValues } = useFormikContext<SettingsData>();
  const isTimezoneChanged = values.timezone !== initialValues.timezone;

  if (isTimezoneChanged) {
    return (
      <Fade in={true}>
        <Alert severity="warning" className="mb-6 rounded-lg!" icon={<InfoIcon fontSize="inherit" />}>
          <AlertTitle>Timezone Changed</AlertTitle>
          Changing the <strong>Business Timezone</strong> will shift your availability hours. 
          Existing bookings will remain at their original UTC time, but might fall outside your new business hours.
        </Alert>
      </Fade>
    );
  }
...
const GeneralSettings = () => {
  ...
  // Timezone Confirmation State
  const [isTimezoneConfirmOpen, setIsTimezoneConfirmOpen] = useState(false);
  const [pendingValues, setPendingValues] = useState<SettingsData | null>(null);
  ...
  // --- Core Logic: Execute Update ---
  const executeUpdate = useCallback(async (values: SettingsData) => {
    try {
        setIsUpdate(true);
        // Simulate a small delay for better UX or API readiness
        setTimeout(async () => {
            await dispatch(updateUserRequest(values));
            setIsUpdate(false);
            setPendingValues(null); // Clear pending
            setIsTimezoneConfirmOpen(false); // Close modal if open
        }, 1500);
    } catch (error) {
        console.error('Update failed:', error);
        setIsUpdate(false);
  ...
  // --- Handler: Form Submit Interceptor ---
  const handleFormSubmit = useCallback((values: SettingsData) => {
    // GHOST BOOKING PREVENTION:
    // If timezone changes, we MUST warn the user because this shifts the grid.
    if (values.timezone !== initialValues.timezone) {
      setPendingValues(values);
      setIsTimezoneConfirmOpen(true);
    } else {
      // If no timezone change, proceed normally
      executeUpdate(values);
    }
  }, [initialValues.timezone, executeUpdate]);

  // --- Handlers: Google & Modals ---
  const handleConnectGoogleCalendar = async () => {
    setIsConnectingGoogle(true);
    try {
      const cookies = new Cookies();
  ...
  return (
    <div>
                  ...
                  <CustomTabPanel value={tabValue} index={0}>
                    <div className="max-w-3xl space-y-10">
                      ...
                      <section>
                        ...
                        {/* ALERT FOR GHOST BOOKING PREVENTION */}
                        <div className="mb-4">
                           <TimezoneChangeWatcher />
                        </div>
      ...
      />
      <ConfirmModal 
        isOpen={isTimezoneConfirmOpen}
        onClose={() => {
            setIsTimezoneConfirmOpen(false);
            setPendingValues(null);
        }}
        onConfirm={() => pendingValues && executeUpdate(pendingValues)}
        title="Update Business Timezone?"
        message= {<>You are changing the business timezone from {initialValues.timezone} to {pendingValues?.timezone}. This will shift your operating hours. <b>Note:</b> Past bookings are stored in UTC and will not move, but they may now visually appear outside your new business hours.</>}
        confirmText="Yes, Update Timezone"
        cancelText="Cancel"
        type="danger" 
        isLoading={isUpdate}
      />
