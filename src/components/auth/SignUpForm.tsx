...
const SignUp: React.FC = () => {
  ...
  // State to track the width of the container for the Google Button
  const [googleBtnWidth, setGoogleBtnWidth] = useState<string | undefined>();
  const googleBtnContainerRef = useRef<HTMLDivElement>(null);

  // Effect to update button width on mount and window resize
  useEffect(() => {
    const handleResize = () => {
      if (googleBtnContainerRef.current) {
        // We calculate the width and update the state
        // Google button max-width is usually capped at 400px by the SDK, 
        // but this ensures it fills the container up to that limit.
        setGoogleBtnWidth(googleBtnContainerRef.current.offsetWidth.toString());
      }
    };
      ...
      {({ isSubmitting }) => (
        <Form className="sm:mt-20" onClick={(e) => e.stopPropagation()}>
          <Grid container spacing={2} mb={2} className="w-full">
            ...
            >
              {/* We pass the calculated width to the GoogleLogin component. 
                 The conditional check ensures we don't render it with 0 width initially.
              */}
              {googleBtnWidth && (
                <GoogleLogin
                  onSuccess={handleGoogleSuccess}
